...
export default async function WriteOffsPage() {
  ...
  // Check which categories user has expenses in
  const { data: tx } = await supabase.from("transactions").select("category").eq("user_id", user.id).lt("amount", 0);
  const userCategories = new Set((tx || []).map((t) => t.category));
  const captured = WRITE_OFFS.filter((w) => userCategories.has(w.id) || userCategories.has("equipment")).length;
  const pct = Math.round((captured / WRITE_OFFS.length) * 100);
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          OnlyFans-specific deductions you might be missing. Every dollar you track = less tax you pay.
        </p>
      </div>
      ...
      <div className="space-y-2">
        {WRITE_OFFS.map((item) => {
          const captured = userCategories.has(item.id);
          return (
            <div key={item.id} className={`rounded-xl border p-4 flex items-start gap-3 transition-all ${
              captured ? "bg-green-50 border-green-200" : "bg-white"
            }`}>
              <span className="text-2xl shrink-0">{item.icon}</span>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-semibold text-staxx-indigo">{item.label}</h3>
                  {captured && <span className="text-xs rounded-full bg-green-100 text-green-700 px-2 py-0.5">✓ captured</span>}
                </div>
      ...
      <div className="rounded-2xl bg-amber-50 border border-amber-200 p-4 text-sm text-amber-800 leading-relaxed">
        <strong>⚠️ Important:</strong> This checklist is educational, not tax advice. Keep receipts for everything.
        Always consult a CPA who understands content creator businesses before filing.
      </div>
    </div>
  );
}
